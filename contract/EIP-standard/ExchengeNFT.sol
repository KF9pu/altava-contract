...
contract ExchangeNFT is ERC721Holder, Ownable, Pausable {
  ...
  uint256 public MAX_TRADABLE_TOKEN_ID = 100000;
  IERC721 public nft;
  ...
  constructor(address _nftAddress, address _quoteErc20Address, address _feeAddress) {
  ...
  function buyToken(uint256 _tokenId) public whenNotPaused {
    ...
    uint256 _commissionValue = price * commission / 100;
    uint256 _sellerValue = price - _commissionValue;

    quoteErc20.safeTransferFrom(msg.sender, _tokenSellers[_tokenId], _sellerValue);
    quoteErc20.safeTransferFrom(msg.sender, feeAddress, _commissionValue);
  ...
  function buyAuction(uint256 _tokenId, uint256 _price) public whenNotPaused {
    require(msg.sender != address(0) && msg.sender != address(this), 'Wrong msg sender');
    require(_asksMap.contains(_tokenId), 'Token not in sell book');
    uint256 price = _asksMap.get(_tokenId);

    require(_price > price, 'Price must be granter than actual price');

    uint256 _commissionValue = _price * commission / 100;
    ...
    _asksMap.remove(_tokenId);
    _userSellingTokens[_tokenSellers[_tokenId]].remove(_tokenId);
    emit Trade(_tokenSellers[_tokenId], msg.sender, _tokenId, _sellerValue);
    delete _tokenSellers[_tokenId];
  }
  ...
  function cancelSellToken(uint256 _tokenId) public whenNotPaused {
    ...
    function setFeeAddress(address _newFeeAddress) external {
      require(msg.sender == feeAddress, 'HolderShib: FORBIDDEN');
